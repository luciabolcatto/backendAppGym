 ...
 * @template {IDatabaseDriver} D current driver type
 */
class EntityManager {
    /**
     * @internal
    ...
    async find(entityName, where, options = {}) {
        ...
        options.schema ?? (options.schema = em._schema);
    ...
    async applyJoinedFilters(meta, cond, options) {
        ...
        if (options.populate) {
            for (const hint of options.populate) {
                ...
                const joined = (hint.strategy || prop.strategy || this.config.get('loadStrategy')) === enums_1.LoadStrategy.JOINED && prop.reference !== enums_1.ReferenceType.SCALAR;
                if (!joined) {
                    continue;
                }
                ...
                if (utils_1.Utils.hasObjectKeys(where2)) {
                    if (ret[hint.field]) {
                        utils_1.Utils.merge(ret[hint.field], where2);
                    }
                    else {
    ...
    getReference(entityName, id, options = {}) {
        options.schema ?? (options.schema = this.schema);
    ...
    async populate(entities, populate, options = {}) {
        ...
        const preparedPopulate = em.preparePopulate(entityName, { populate: populate }, options.validate);
    ...
    preparePopulate(entityName, options, validate = true) {
    ...
    shouldRefresh(meta, entity, options) {
        ...
        if (options.fields) {
            ...
            autoRefresh = meta.comparableProps.some(prop => {
                const inlineEmbedded = prop.reference === enums_1.ReferenceType.EMBEDDED && !prop.object;
    ...
    async tryCache(entityName, config, key, refresh, merge) {
        config ?? (config = this.config.get('resultCache').global);
        if (!config) {
            return undefined;
        }
        const em = this.getContext();
