...
class EntityAssigner {
    ...
    static assignProperty(entity, propName, props, data, options) {
        ...
        if ([enums_1.ReferenceType.MANY_TO_MANY, enums_1.ReferenceType.ONE_TO_MANY].includes(prop?.reference) && entity[prop.name] == null) {
        ...
        if (options.convertCustomTypes && customType && prop.reference === enums_1.ReferenceType.SCALAR && !Utils_1.Utils.isEntity(data)) {
            value = prop.customType.convertToJSValue(value, options.platform);
        }
        if ([enums_1.ReferenceType.MANY_TO_ONE, enums_1.ReferenceType.ONE_TO_ONE].includes(prop?.reference) && value != null) {
            ...
            if (options.updateNestedEntities && entity.hasOwnProperty(propName) && Utils_1.Utils.isEntity(entity[propName], true) && Utils_1.Utils.isPlainObject(value)) {
                ...
                if (options.updateByPrimaryKey) {
                    ...
                    if (pk) {
                        ...
                        const sameTarget = (0, wrap_1.helper)(ref).getSerializedPrimaryKey() === unwrappedEntity.__helper.getSerializedPrimaryKey();
                        if ((0, wrap_1.helper)(ref).isInitialized() && sameTarget) {
        ...
        if (prop?.reference === enums_1.ReferenceType.SCALAR && enums_1.SCALAR_TYPES.includes(prop.type) && (prop.setter || !prop.getter)) {
            return entity[prop.name] = validator.validateProperty(prop, value, entity);
        }
        if (prop?.reference === enums_1.ReferenceType.EMBEDDED && EntityAssigner.validateEM(options.em)) {
            return EntityAssigner.assignEmbeddable(entity, value, prop, options.em, options);
        }
        if (options.mergeObjects && Utils_1.Utils.isPlainObject(entity[propName]) && Utils_1.Utils.isPlainObject(value)) {
