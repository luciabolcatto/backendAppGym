...
function sanitizedEntrenadorInput(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
async function findAll(req: Request, res: Response) {
  try {
    const entrenadores = await em.find(
      Entrenador,
      {},
      { populate: ['actividades', 'clases'] }
    );
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    const entrenador = await em.findOneOrFail(
      Entrenador,
      { id },
      { populate: ['actividades', 'clases'] }
    );
    res
      .status(200)
      .json({ message: 'entrenador encontrado', data: entrenador });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
}
...
async function remove(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({ message: 'entrenador borrado' });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
}
