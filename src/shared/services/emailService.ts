 ...
 * @param email Email del destinatario (ej: pedro@mail.com)
...
export async function sendRecoveryCode(
    ...
    htmlContent: `
      ...
      <head>
        <style>
          ...
          .code-box { background-color: #fff; padding: 20px; text-align: center; border-radius: 5px; margin: 20px 0; border: 2px solid #4CAF50; }
          .code { font-size: 32px; font-weight: bold; letter-spacing: 5px; color: #4CAF50; }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Detalles del error de Brevo:', errorData);
      throw new Error('Fallo la respuesta de Brevo');
    }

    console.log(`✅ Código de recuperación enviado a ${email} vía Brevo`);
  } catch (error) {
