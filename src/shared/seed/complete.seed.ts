...
async function seedCompleto() {
  ...
  const actividades = [
    em.create(Actividad, {
      ...
      nombre: 'Yoga',
      descripcion:
        'Práctica de yoga para principiantes y nivel intermedio. Conecta tu mente y cuerpo.',
      cupo: 15,
    }),
    em.create(Actividad, {
      ...
      descripcion:
        'Práctica avanzada de yoga con posturas complejas y técnicas de respiración.',
      cupo: 12,
    }),
    em.create(Actividad, {
      ...
      nombre: 'Spinning',
      descripcion:
        'Entrenamiento cardiovascular intenso en bicicleta estática con música motivadora.',
      cupo: 20,
    }),
  ];
  ...
  const membresias = [
    ...
    em.create(Membresia, {
      nombre: 'Premium',
      descripcion:
        'Acceso completo + clases premium + asesoramiento nutricional.',
      precio: 12000,
      meses: 3,
    }),
    em.create(Membresia, {
  ...
  const crearContrato = (
    ...
    if (estado === EstadoContrato.PAGADO || estado === EstadoContrato.VENCIDO) {
      ...
      const fechaPagoCalculada = new Date(
        fechaInicio.getTime() + diasPago * 24 * 60 * 60 * 1000
      );
      // Asegurar que el pago sea ANTES DE AHORA
      fechaPago =
        fechaPagoCalculada < ahora
          ? fechaPagoCalculada
          : new Date(
              ahora.getTime() -
    ...
    if (estado === EstadoContrato.CANCELADO) {
      ...
      if (fuePagado) {
        ...
        const fechaCancelacionCalculada = new Date(
          fechaPago.getTime() + diasCancelacion * 24 * 60 * 60 * 1000
        );
        // Asegurar que la cancelación sea ANTES DE AHORA
        fechaCancelacion =
          fechaCancelacionCalculada < ahora
            ? fechaCancelacionCalculada
            : new Date(
                ahora.getTime() -
                  Math.floor(Math.random() * 30 + 1) * 24 * 60 * 60 * 1000
              ); // 1-30 días atrás
        ...
        const fechaCancelacionCalculada = new Date(
          fechaInicio.getTime() + diasCancelacion * 24 * 60 * 60 * 1000
        );
        // Asegurar que la cancelación sea ANTES DE AHORA
        fechaCancelacion =
          fechaCancelacionCalculada < ahora
  ...
  ); // Premium PAGADO (vence en enero 2026)
  ...
  ); // Básica PAGADO (vence en 10 días)
  ...
  ); // Básica PAGADO (vence en 15 días)
  fechaInicio = new Date(fechaInicio.getTime() + 31 * 24 * 60 * 60 * 1000); // +1 mes (encadenado exacto)
  contratos.push(
    crearContrato(
      usuarios[5],
  ...
  ); // Básica PAGADO (vence en 20 días)
  fechaInicio = new Date(fechaInicio.getTime() + 31 * 24 * 60 * 60 * 1000); // +1 mes (encadenado exacto)
  contratos.push(
    crearContrato(
      usuarios[6],
  ...
  ); // Premium PAGADO (vence en diciembre 2025)
  ...
  ); // Premium PAGADO (válido hasta enero)
  ...
  ); // Básica PAGADO (válido hasta noviembre)
  ...
  ); // Estudiante PAGADO (válido hasta noviembre)
  ...
  contratos.push(
    em.create(Contrato, {
      ...
      fecha_hora_fin: new Date(
        fechaVencidaInicio.getTime() + 30 * 24 * 60 * 60 * 1000
      ...
      fechaPago: new Date(
        fechaVencidaInicio.getTime() + 2 * 24 * 60 * 60 * 1000
      ), // Pagado 2 días después del inicio
    })
  );
  ...
  contratos.push(
    em.create(Contrato, {
      ...
      metodoPago: 'Efectivo',
      fechaPago: new Date(
        fechaVencidaInicio.getTime() + 1 * 24 * 60 * 60 * 1000
      ), // Pagado 1 día después del inicio
    })
  );
  ...
  contratos.push(
    em.create(Contrato, {
      ...
      fechaPago: new Date(
        fechaVencidaInicio.getTime() + 3 * 24 * 60 * 60 * 1000
      ), // Pagado 3 días después del inicio
    })
  );
  ...
  console.log(
    '🧪 Agregando clases de prueba para actualización de reservas...'
  );
  ...
  reservas.push(
    em.create(Reserva, {
      fecha_hora: new Date(
        clasePasadoPrueba.fecha_hora_ini.getTime() - 2 * 60 * 60 * 1000
      ), // Reservada 2h antes
    ...
    em.create(Reserva, {
      fecha_hora: new Date(
        clasePasadoPrueba.fecha_hora_ini.getTime() - 1 * 60 * 60 * 1000
      ), // Reservada 1h antes
      estado: EstadoReserva.PENDIENTE, // 🚨 ESTADO PENDIENTE para clase pasada
  ...
  console.log(
    `   - 2 reservas PENDIENTES para clase a 40min (deben → PENDIENTE)`
  );
  console.log(
    `   - 2 reservas PENDIENTES para clase a 25min (deben → CERRADA)`
  );
      ...
      if (claseYaPaso) {
        ...
        if (fueCancelada) {
          ...
          const tiempoAleatorio =
            Math.floor(Math.random() * (maxTiempo - minTiempo)) + minTiempo;
        ...
        if (falta30Min) {
          ...
          const tiempoAleatorio =
            Math.floor(Math.random() * (maxTiempoAtras - minTiempoAtras)) +
            minTiempoAtras;
  ...
  console.log(
    '✅ Agregadas 5 clases de prueba: 3 sin cupo + 2 con 1 solo cupo (SIN RESERVAS)'
  );
...
- ${
  ...
  } Clases (incluye 5 clases de prueba SIN RESERVAS)
